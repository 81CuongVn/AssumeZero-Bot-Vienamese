...
function handleMessage(err, message, external = false, api = gapi) { // New message received from listen()
    if (message && message.threadID && !err) {
        ...
        utils.getGroupInfo(message.threadID, (err, info) => {
            if (err || !info) {
                console.log(`Error retrieving group data for ${message.threadID}: ${err} , kệ dòng này`);
            } else {
                // Welcome new members
                ...
                const senderId = message.senderID;
                    if ((message.type == "message" || message.type == "message_reply") && senderId != config.bot.id) { // Sender is not banned and is not the bot
